...
pub fn cleanup_old_backups(app_config: &AppConfig) {
    ...
    if let Some(keep_last) = retention_config.keep_last {
        files_to_delete.extend(backup_files.iter().rev().skip(keep_last).map(|e| e.entry.path()));
    }

    backup_files.iter().for_each(|file| debug_log!("File date: {}", file.date));

    if let Some(max_age) = retention_config.max_age {
        let cutoff = now - max_age;
        debug_log!("Backup cleanup by age: now: {now}, cutoff date: {cutoff}");
        files_to_delete.extend(backup_files.iter()
...
fn list_backup_files(app_config: &AppConfig) -> Vec<BackupFile> {
    ...
    let mut backup_files: Vec<_> = a.into_iter().filter_map(|e| e.ok())
        .filter_map(|entry| {
            let filename = entry.file_name().to_str().unwrap().to_owned();
            let Some(caps) = filename_regex.captures(&filename) else { return None };
            let Some(date) = parse_iso_datetime(&caps[1]) else { return None };
            Some(BackupFile { entry, date })
        })
        .collect();

    backup_files.sort_by(|a, b| a.date.cmp(&b.date));
...
fn parse_iso_datetime(date_str: &str) -> Option<chrono::DateTime<chrono::Local>> {
    NaiveDateTime::parse_from_str(date_str, ISO_DATETIME_FORMAT).ok()?
